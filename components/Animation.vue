...
export default {
  ...
  mounted() {
    ...
    function init() {
      ...
      let states = [];
      loader.load("nuxtwebgl/female.vtk", function(mesh) {
        console.log(mesh);
        ...
        // mesh.vertices - массив из 100 000 вершин x,y,z
        ...
        for (var i = 0; i <= numVertices * 3; i = i + 3) {
          // наполним трёхмерный куб частицами - 3 координаты
        ...
        // начальное состояние для анимации объкта на странице
        ...
        vm.material = new THREE.RawShaderMaterial({
          uniforms: {
            dot: {
              type: "t",
              value: new THREE.TextureLoader().load("nuxtwebgl/reddot.png")
            },
            blend: { type: "f", value: 0 },
            size: { type: "f", value: 2.1 }
          },
          // vec3 result = source*(1. - blend) + blend*target;
          // линейная интерполяция между состояниями source- куб target - девушка
        ...
        // поворот поинтов, чтобы сцена была по центру
